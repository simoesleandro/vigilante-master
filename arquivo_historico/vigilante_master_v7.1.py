...
# ==========================================
# 3. MÓDULOS DE EXTRAÇÃO (TJRJ, STF, TSE) - Versão DPI Normal
# ==========================================
def extrair_playwright(p_instance, id_nome, url, tribunal_label):
    print(f"📡 Capturando {tribunal_label}: {id_nome}...")
    
    # Mantemos o truque da Janela Fantasma para não roubar foco
    nav = p_instance.chromium.launch(
        headless=False,
        args=['--window-position=-32000,-32000']
    )
    
    # A MÁGICA ESTÁ AQUI: Mudamos device_scale_factor para 1 (resolução nativa, sem zoom)
    ...
    try:
        ...
        if tribunal_label == "TJRJ":
            ...
            # Como tiramos o zoom, precisamos ajustar levemente a área de recorte (clip)
            # para capturar o mesmo conteúdo no tamanho original.
