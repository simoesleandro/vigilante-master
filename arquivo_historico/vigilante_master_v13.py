...
import html
import traceback
import sqlite3 # <--- NOVA IMPORTAÇÃO PARA O BANCO DE DADOS
...
# ==========================================
# 2. GERENCIAMENTO DO BANCO DE DADOS (NOVO)
# ==========================================
def inicializar_banco():
    ...
    try:
        ...
        # Tenta adicionar a coluna ultimo_andamento caso ela não exista (para bancos já existentes)
        try:
...
# ==========================================
# 3. INTELIGÊNCIA ARTIFICIAL - RESUMO EVOLUTIVO (NOVO)
# ==========================================
...
def extrair_tse_stealth(id_nome, url, numero):
    ...
    try:
        ...
        # Espera o captcha ser resolvido de forma ativa (polling)
        card_alvo = None
...
# ==========================================
# 6. INTERAÇÃO E INTELIGÊNCIA COM BANCO DE DADOS
# ==========================================
...
def processar_clique_botao(call):
    ...
    try:
        ...
        # 👇 BUSCA OS DADOS FRESCOS DIRETO DO BANCO DE DADOS
...
def processar_texto_humano(message, pid, proc):
    """Grava o texto que você digitou no Telegram direto na tabela de histórico do SQLite"""
    if not message.text:
...
def tarefa_ia_resumo(message, pid, proc):
    """Lê o resumo, as interações do advogado no SQLite e gera a análise estratégica"""
    try:
...
# ==========================================
# 7. MOTOR DE MONITORAMENTO PRINCIPAL
# ==========================================
FALHAS_CONSECUTIVAS = {}
...
def comparar_e_enfileirar(proc, tribunal, txt, img):
    ...
    # Sucesso na leitura, reseta o contador de falhas
    FALHAS_CONSECUTIVAS[pid] = 0
    
    print(f"   🔍 {pid}: Comparando histórico...")
    txt_novo = txt.strip()
    
    # Busca o último andamento direto do processo passado
    txt_antigo = proc.get("ultimo_andamento")
    if txt_antigo:
        txt_antigo = txt_antigo.strip()
    
    if not txt_antigo:
        # Se não há último andamento no banco, salva o atual e encerra (estado inicial)
        try:
            conn = sqlite3.connect('memoria_vigilante.db')
            cursor = conn.cursor()
    ...
    if txt_novo != txt_antigo:
        ...
        # 👇 ATUALIZA O RESUMO NO BANCO DE DADOS EM SEGUNDO PLANO
        ...
        # 👇 ATUALIZA O ÚLTIMO ANDAMENTO NO BANCO DE DADOS
        try:
            conn = sqlite3.connect('memoria_vigilante.db')
            cursor = conn.cursor()
            cursor.execute("UPDATE processos SET ultimo_andamento = ? WHERE pid = ?", (txt_novo, pid))
...
def iniciar_vigilancia():
    print("🚀 Vigilante Master v13.0 Inicializando...")
    
    # 💾 GARANTE QUE O BD EXISTE E TEM OS DADOS INICIAIS ANTES DE RODAR
    inicializar_banco()
    
    try:
        bot.send_message(ADMIN_ID, "🚀 <b>Vigilante Master v13.0 Online!</b>\nBanco de Dados e IA Evolutiva Ativados.", parse_mode="HTML")
    except: pass
    ...
    while True:
        ...
        # 👇 LÊ OS TRIBUNAIS DIRETAMENTE DO SQLITE A CADA CICLO
        ...
        if cnt % 10 == 0:
            # Executa a fila de processos do TSE de forma assíncrona e sequencial para não travar o loop principal
